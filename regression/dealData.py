...
def changedata2(Alldata, Outputpath):
    ...
    for i in range(len(Alldata)):
        ...
        if (int(date[1]) == 12 and (int(date[2]) == 24 or int(date[2]) == 25)) is False:
        ...
        line[88 + int(float(row[4])/2.500001)] = "1"
        if int(date[1]) == 12 and (int(date[2]) >= 20 and int(date[2]) <= 31): #增加12月下旬的划分，拟合12月（本来有36列 只保留1列 防止跑不动）
            line[109] = "1"
        if int(date[1]) == 12 and (int(date[2]) >= 24 and int(date[2]) <= 25): #将假期调整周末
            line[49] = "1"
        if int(date[1]) == 12 and (int(date[2]) >= 22 and int(date[2]) <= 27) and (int(date[2]) != 25):#圣诞节当周 
            line[110] = "1"
        if int(date[1]) == 12 and int(date[2]) == 25: #圣诞节当天
            line[111] = "1"
        if n == 8:
            line[112] = row[7]
