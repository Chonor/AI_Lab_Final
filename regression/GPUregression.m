function  []=GPUregression()
    ...
    Test =load('test.txt');
    load('GPUseed.mat');%加载种子
    [train_row, train_col] = size(Train);
    %%设置节点数[输入层,xx,...,xx,输出层],参数设置
    Node = [train_col-1 ,300,300,1];
    layers = size(Node,2);
    step = [0.01, 0.015,0.0200];
    lambda = [0.003 0.005 0.009];
    alpha=0.25;
    iter = 1400;
    ...
    G_val=gpuArray(train(7905:8645,:));
    G_val_ans=gpuArray(train_ans(7905:8645,:));
...
%     G_val_ans=G_val_ans3;
    ...
    [MSE,corr,MSE_val,corr_val,out1,out2,best_w,best_b,ow,ob]= BPNN(G_train, G_val,ow,ob,G_iter,G_Node, layers,G_step,G_lambda,G_alpha,G_train_ans,G_val_ans,gpuArray(single(size(G_train_ans,1))),gpuArray(single(size(G_val_ans,1))));
    %--------------------------以下为作图调参区 现在不用了
    %     figure;
    ...
    plot(1:size(best0,1), best0, 'b');
    csvwrite('66_v1.csv',round(abs(best0)));
end
function [MSE,corr,MSE_val,corr_val, out1, out2,best_w,best_b,ow,ob] = BPNN(train, val, w,theta,iter, Node, layers, step, lambda,alpha,train_ans, val_ans,train_row,val_row)
    Min_mse=10000;
    %    w = cell(1, layers-1);
    best_w=w;
    out = cell(1, layers - 1);
