function []=GPUmulticlassification()
%%数据载入预处理
    load('train_test.mat');
    all=train_test;
...
function [w,theta,best_drop,ow,ob,odrop] = BPNN(train,train_ans,val,val_ans,iter, Node, layers, step, lambda)
    ...
    low_acc =gpuArray.zeros(1,iter,'single');
    mid_acc =gpuArray.zeros(1,iter,'single');
    hig_acc =gpuArray.zeros(1,iter,'single');
    beta1 = 0.9;
    beta2 = 0.999;
    p=0.5;
    ...
    for i = 1 :layers-1
        ...
        m{i} = gpuArray.zeros(Node(i), Node(i+1),'single');
        v{i} = gpuArray.zeros(Node(i), Node(i+1),'single');
        theta{i} = gpuArray.rand(1,Node(i+1),'single')*0.2-0.1;
        drop{i} = gpuArray.rand(1,Node(i+1), 'single')<p;
    end
    ow=w;
    ob=theta;
    odrop=drop;
    %%--------------迭代----------------------------
    for it = 1 : iter
                waitbar(cnt/lengths,h,num2str(cnt));
        cnt=cnt+1;
        %%------------计算各层结果--------------------
        %输入层->隐藏层
